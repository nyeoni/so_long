...
/*   Updated: 2022/01/29 18:43:15 by nkim             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void handle_wall(t_game *game, int offsetX, int offsetY)
{
	...
	if (offsetX > 0)
		...
		row += 1;
	if (map[row][col] != '1' || (!((game->player.y + offsetY) % TILE_SIZE)
...
void handle_collect(t_game *game, int offsetX, int offsetY)
{
	...
	{
		ft_lstc_delete(&game->collect.collections, game->player.x + offsetX, game->player.y + offsetY);
		mlx_put_image_to_window(game->mlx_ptr, game->win_ptr,
				game->tiles.ground, game->player.x + offsetX, game->player.y + offsetY);
	}
}
